...
describe('Education — end-to-end user-manual walkthrough', () => {
  ...
  it('walks the full education domain', () => {
    ...
    cy.get('#attendanceGrade', { timeout: 10000 }).find('option').should('have.length.greaterThan', 1)
    cy.get('#attendanceGrade').select('Grade 1', { force: true })
    cy.wait(STEP)
    ...
    cy.get('#alertConsumers').select(['Students'], { force: true })
    cy.get('#alertType').select(['Notice Board'], { force: true })
    cy.get('#alertDeliveryChannel').select(['Email'], { force: true })
    cy.get('#alertDeliveryPeriod').select(['Daily'], { force: true })
    cy.get('#alertDeliveryType').select('Manual', { force: true })
    cy.get('#alertHeading').clear().type('Holiday Notice')
    cy.get('#alertMessage').clear().type('School closed Friday.')
