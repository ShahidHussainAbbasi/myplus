 ...
 *
 * Requires the seeded demo user appointment@myplus.com (auth-service SetupDataLoader, dev seed flag)
...
describe('Slice-18 — APPOINTMENT user lands on /appointmentDashboard', () => {
  it('logs in and is routed to its own module dashboard', () => {
    cy.visit('/login')
    cy.get('input[name="username"]').type('appointment@myplus.com')
    cy.get('input[name="password"]').type('Appoint@2025!')
