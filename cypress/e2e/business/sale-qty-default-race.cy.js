...
describe('QTY-RACE-1 — the stock answer never overwrites a typed quantity', () => {
  ...
  it('⭐⭐ 1 — quantity typed while /productStock is in flight survives its answer', () => {
    cy.seedProduct({ name: `QR1_${uniq()}`, sellingPrice: 100, stock: 20 }).then(({ productId }) => {
      open(productId)

      /*
       * THE EXACT ORDER THAT FAILED: the cashier CLEARS the box, the late answer lands in that gap, THEN they
       * type. (Typing before the answer never failed — the default only ever filled an EMPTY box — so a case
       * built that way would pass with the fix removed.) The request is held until the box has been cleared.
       */
      let release
      ...
      cy.get('#sellQuantity').clear({ force: true })            // the cashier empties the box…
      cy.then(() => release())                                  // …the late answer lands in the gap…
      cy.wait('@stock')
      cy.wait(500)                                              // …its success handler runs…
      cy.get('#sellQuantity').should('have.value', '')          // ⭐ and leaves the touched box alone
      cy.get('#sellQuantity').type('5', { force: true })        // …then they type
