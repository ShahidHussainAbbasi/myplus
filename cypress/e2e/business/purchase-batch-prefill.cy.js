  ...
  it('getStockByBatch returns inventory on-hand + last purchase rate + master sell price', () => {
    ...
    cy.then(() => {
      cy.request({ method: 'POST', url: '/addPurchase', form: true, failOnStatusCode: false,
        body: { productId, quantity: 6, 'stock.batchNo': batch, 'stock.bpurchaseRate': 10, 'stock.bsellRate': 15,
    ...
    cy.then(() => {
      const poll = (t) => cy.request('/getStockByBatch?batchNo=' + batch + '&productId=' + productId).then((r) => {
