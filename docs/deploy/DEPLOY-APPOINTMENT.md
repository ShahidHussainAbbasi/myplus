...
docker compose exec mysql sh -c \
  'exec mysqldump -uroot -p"$MYSQL_ROOT_PASSWORD" myplusdb_appointment' > appt-$(date +%F).sql
```

Include `myplusdb_auth`, and — given the note above — `myplusdb` if the legacy `user` table is still in play.
