...
- [x] **Attendance + Fee Collection list reads are now branch-scoped.** These records carry no `school_id` of their own but each is *for a student* (by `enrollNo`), so their branch is the student's. `getUserA`/`getAllA`/`getUserFc`/`getAllFc` now filter to rows whose enrollNo belongs to a student in the caller's accessible branches (owner/super or no-grants ⇒ org-wide, unchanged; orphaned null-enroll rows stay per the store_id-NULL convention). Closes the within-tenant leak (a teacher at one campus was seeing every campus's attendance/fees). Gated by T9h. `FeeCollectionController` gained a `StudentRepository` inject; `AttendanceController` already had one.
  - [ ] **Residual (marking flow):** `getClassRoster` / `markAttendanceBulk` still iterate `studentRepository.findScoped` (org-wide) — the roster/marking path, not the list-display leak. Should reuse the same branch-scoped student set; deferred as a write-path change needing its own care.
  - [ ] **Org-level entities — policy decision needed:** `Guardian`, `Staff`, `Subject`, `Discount` have **no** branch link at all. `Subject`/`Discount` are plausibly org-wide (curriculum, fee policy). `Staff` arguably belongs to a branch (needs its own `school_id`). `Guardian` may span campuses (a parent with children at two branches) — the open question. Left org-wide until decided.
