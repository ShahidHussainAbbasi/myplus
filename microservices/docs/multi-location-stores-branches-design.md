...
- [x] **Attendance is branch-level; fee collection is org-wide by default (configurable).** Both records carry no `school_id` but each is *for a student* (by `enrollNo`), so a branch can be derived. Per the owner's stated model, the two concerns differ:
  ...
  - [ ] **Org-level entities — policy decision still open:** `Guardian`, `Staff`, `Subject`, `Discount` have **no** branch link. `Subject`/`Discount` plausibly org-wide; `Staff` arguably per-branch (needs its own `school_id`); `Guardian` may span campuses. The `FeeSetting`-flag pattern above is the template for making any of these configurable when decided.
