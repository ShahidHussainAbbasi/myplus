...
**POS gaps:** the owner Configuration screen now exists on `common-settings`, with `pos.receipt.showTaxBreakdown` **behaviour-wired** (2026-07-26 — the flag rides on `/getReceipt` and `receipt.js` collapses per-rate rows when off; gate `business/receipt-tax-breakdown.cy.js`). A `pos.sale.negativeStockAllowed` toggle was **removed by design** (2026-07-26): most "Insufficient stock" cases are expired/held batches excluded from *sellable* — a data fix (fresh batch / correct expiry), not a true shortage — and deliberate overselling into negative inventory isn't a policy this system offers. Legacy Tax/Stores/period-lock remain bespoke screens to fold in. Plus the standing GL-reversal-drift items tracked in `pos-retail-standards-audit.md`.
