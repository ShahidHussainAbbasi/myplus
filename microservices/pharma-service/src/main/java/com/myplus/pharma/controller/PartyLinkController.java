...
import com.myplus.pharma.service.PartyBridgeService;

import lombok.RequiredArgsConstructor;

/**
 * One-time admin backfill of the contact view's role index (P4) for prescriptions bridged before P4 (they carry a
 * party_id already, so the bridge's skip-guard means they never bridge again). Batched by id cursor, idempotent — call
 * until {@code remaining} is 0. Owner/admin-gated; not on any automatic path.
 */
@RestController
